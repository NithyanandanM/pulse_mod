...
class pulse_email_vars {
    ...
    /**
     * Email vars are udpated for the use of notification or content.
     *
     * @var string
     */
    ...
    /**
     * Scheduled user data record.
     *
     * @var object
     */
    public $scheduleuser = null;
    ...
    public function __construct($user, $course, $sender, $pulse, $condition = null, $type = 'notification') {
        ...
        $wwwroot = $CFG->wwwroot;
        $this->pulse = $pulse;
        if (!isset($this->pulse->options) && !empty($this->pulse->id)) {
            $this->pulse->options = (object) \mod_pulse\options::init($this->pulse->id)->get_options();
        }
        ...
        foreach ($plugins as $component => $pluginbase) {
            $this->assignment = method_exists($pluginbase, 'get_assignment_extension')
                ? $pluginbase->get_assignment_extension($this->course->id, $this->user->id) : null;
        }
    ...
    public static function vars($automation = false) {
        ...
        $result = [
            ...
            // Schedule user fields.
            'Scheduleuser' => self::schedule_user_profile_fields(),
        ];
    ...
    public function __get($name) {
        if (isset($name)) {
            ...
            } else if ($this->get_extend_vars_list($name)) {
                return $this->get_extend_vars_data($name); // Call the method.
    ...
    /**
     * Set the schedule user data.
     *
     * @param mixed $scheduleuser Schedule user data object.
     * @return void
     */
    public function set_scheduleuser($scheduleuser) {
        $this->scheduleuser = $scheduleuser;
    }

    /**
     * Get the extended variables list.
     *
     * @param string $name The name of the variable.
     * @return bool True if the variable is in the list, false otherwise.
     */
    ...
    /**
     * Get the extended variables data.
     *
     * @param string $name The name of the variable.
     * @return mixed The value of the extended variable.
     */
    ...
    public function get_user_enrolment() {
        ...
        if (!empty($enrolments)) {
            ...
            $courseduedate = helper::create()->timetable_details(
                'coursedue',
                $this->orgcourse,
                $this->user->id,
                $this->coursecontext,
    ...
    public function training_data() {
        ...
        $percentage = \core_completion\progress::get_course_progress_percentage($this->orgcourse, $this->user->id);
        $progress = !empty($percentage) ? $percentage : 0;
        $eventdates = helper::create()->timetable_details(
            'eventdates',
            $this->orgcourse,
            $this->user->id,
            $this->coursecontext,
        ...
        $upcomingmods = helper::create()->timetable_details(
            'upcomingmods',
            $this->orgcourse,
            $this->user->id,
            $this->coursecontext,
        ...
        $activityduedate = helper::create()->timetable_details(
            'activityduedate',
            $this->orgcourse,
            $this->user->id,
            $this->coursecontext,
    ...
    /**
     * List of schedule user profile fields and custom fields.
     *
     * @return array List of schedule user profile fields.
     */
    public static function schedule_user_profile_fields() {
        global $DB, $CFG;
        ...
        if ($fields === null) {
            ...
            array_walk($fields, function (&$value) {
                $value = 'Scheduleuser_' . ucwords($value);
            });

            $fields = array_values($fields);

            $removefields = [
                'Scheduleuser_confirmed', 'Scheduleuser_policyagreed', 'Scheduleuser_deleted',
    ...
    public function get_calendar() {
        ...
        $upcomingactivities = helper::create()->timetable_details(
            'upcomingmods',
            $this->orgcourse,
            $this->user->id,
            $this->coursecontext,
