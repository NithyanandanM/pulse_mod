...
Feature: Pulse group condition
  ...
  Scenario: New condition: Course group - No group
    ...
    And the following should exist in the "reportbuilder-table" table:
      | Course full name | Message type | Subject                        | Full name      | Time created                    | Scheduled time                  | Status |
      | Course 1         | Template1    | No Group upcoming notification | Teacher User 1 | ##now##%A, %d %B %Y, %I:%M %p## | ##now##%A, %d %B %Y, %I:%M %p## | Queued |
      | Course 1         | Template1    | No Group upcoming notification | student User 4 | ##now##%A, %d %B %Y, %I:%M %p## | ##now##%A, %d %B %Y, %I:%M %p## | sent   |
