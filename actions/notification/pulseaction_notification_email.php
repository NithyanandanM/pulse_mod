...
use pulseaction_notification\local\custom_mail;
...
class pulseaction_notification_email extends message_output_email {
     ...
     * - Allowed the nologin users created by pulse custom emails recipients to receive emails .
     *
    ...
    public function send_message($eventdata) {
        ...
        // --PULSE MODIFICATION--
        // Allow nologin users created by pulse custom email recipients to receive emails.
        $ispulsecustomrecipient = custom_mail::is_pulse_custom_recipient_user($eventdata->userto->id);
        if (
            ($eventdata->userto->auth === 'nologin' && !$ispulsecustomrecipient)
    ...
    public function email_to_user(
        ...
        // -- PULSE MODIFICATION --
        // Allowed nologin users created by pulse custom email recipients to receive emails .
        $ispulserec = custom_mail::is_pulse_custom_recipient_user($user->id);
        if ((isset($user->auth) && $user->auth == 'nologin' && !$ispulserec) || (isset($user->suspended) && $user->suspended)) {
            return true;
        }
        ...
        if (substr($user->email, -8) == '.invalid') {
            debugging(
                "email_to_user: User $user->id (" . fullname($user) . ") email domain ($user->email) is invalid! Not sending."
            );
