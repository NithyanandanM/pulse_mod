...
class instances extends templates {
    ...
    public function include_conditions_data(&$instance) {
        ...
        $conditiondata = $instance->template->triggerconditions ?? $instance->triggerconditions;
    ...
    public function duplicate() {
        ...
        // Create the item.
        $newinstanceid = self::manage_instance($instance);

        // Duplicate the actions filearea.
        // Currently the conditions are not have editors, in future we will add the editor for conditions.
        $actionplugins = \mod_pulse\plugininfo\pulseaction::get_list();
        foreach ($actionplugins as $component => $pluginbase) {
            if (method_exists($pluginbase, 'duplicate_fileareas')) {
                $pluginbase->duplicate_fileareas($this->instanceid, $newinstanceid);
            }
        }
    }

    /**
     * Include the actions report icon for the given row.
     *
     * @param array $actions The array of actions.
     * @param stdClass $row The row object.
     * @param moodle_url $listurl The URL of the list page.
     * @return void
     */
    public static function include_actions_reportsview(&$actions, $row, $listurl) {
        global $DB;

        // Fetch the list of enabled action plugins.
        $actionplugins = \mod_pulse\plugininfo\pulseaction::get_list();
        foreach ($actionplugins as $component => $pluginbase) {
            $reportsource = $pluginbase->get_report_source();
            // Create template instance. Actions are performed in template instance.
            $reportid = self::get_reportid($reportsource, $component);
            $report = manager::get_report_from_id($reportid);

            if (permission::can_view_report($report->get_report_persistent())) {
                $pluginbase->include_reports_view($actions, $row, $listurl);
            }
        }
    }
    ...
    public function get_report_url() {
        $report = optional_param('report', 'notification', PARAM_ALPHA);

        $actionplugins = \mod_pulse\plugininfo\pulseaction::get_list();
        foreach ($actionplugins as $component => $pluginbase) {
            if ($pluginbase->get_component() === $report) {
                $reportsource = $pluginbase->get_report_source();
     ...
     *
     * @param string $reportsource The report source.
     * @param string $component The component name.
     *
     * @return int
     */
    public static function get_reportid($reportsource, $component) {
        global $DB;
        ...
        if ($report = $DB->get_record('reportbuilder_report', $data)) {
            ...
            $langexist = get_string_manager()->string_exists('automationreportname', 'pulseaction_' . $component);

            if ($langexist) {
                $data['name'] = (new lang_string('automationreportname', 'pulseaction_' . $component))->out();
            }
    ...
    public static function manage_instance($formdata) {
        ...
        foreach ($record->condition as $component => $status) {
            ...
            $condition->process_instance_save($instanceid, $record->condition[$component], $templatedata);
        }
        ...
        foreach ($actionplugins as $component => $pluginbase) {
            try {
                ...
                // Log the exception and continue with next action.
                \core\notification::error(get_string('actionstoresaveerror', 'pulse', $e->getMessage()));
            }
        }
    ...
    protected static function remove_override_values($fields, $instanceid) {
        ...
        if (!empty($fields)) {
            ...
            foreach ($actions as $component => $pluginbase) {
                ...
                $actiondata = array_fill_keys(array_keys((array) $actiondata), null);

                if (empty(array_filter($actiondata))) {
                    continue;
                }
